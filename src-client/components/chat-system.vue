<script setup lang="ts">
    ...
    // For now, we'll use a hardcoded roomId - you can make this dynamic later
    const currentRoomId = ref("cf9d62dc-8c56-4eae-b074-2790c19f9a17"); // TODO: Make this dynamic based on selected room
    ...
    const loadMessages = async () => {
      try {
        const supabaseMessages = await getMessages(currentRoomId.value);
    ...
    const sendMessage = async () => {
      ...
      try {
        ...
        const sentMessage = await sendMessageToSupabase(currentRoomId.value, messageContent);
    ...
    const setupSubscription = () => {
      ...
      unsubscribe = subscribeToMessages(currentRoomId.value, (newMessage: Message) => {
    ...
    // Watch for room changes (if you make roomId dynamic)
      ...
      <UDrawer v-model="isOpen" direction="right">
        ...
        <template #content>
          <div class="flex flex-col h-full max-w-lg w-full overflow-hidden">
            <!-- Header -->
            <div
              class="flex items-center justify-between p-4 border-b dark:border-gray-800 flex-shrink-0"
            >
              <div class="flex items-center gap-2">
                <UIcon name="i-lucide-users" class="w-5 h-5" />
                <h2 class="text-lg font-semibold">
                    Shard Chat
                    <UBadge label="Beta" variant="subtle" color="warning" icon="i-lucide-flask-conical" size="sm" class="ml-4" />
                </h2>
              </div>
              <UButton
                @click="closeDrawer"
                ...
                >
                  ...
                  >
                    <div class="flex flex-col gap-1">
                      ...
                      <p
                        class="text-sm whitespace-pre-wrap break-words overflow-wrap-anywhere min-w-0 pl-6"
                      >
