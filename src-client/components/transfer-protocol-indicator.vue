<script setup lang="ts">
  ...
  const {
    ...
    ftpIps,
    primaryFtpIp,
  } = storeToRefs(transferProtocolStore);
  ...
  <template>
    ...
    <UModal v-model:open="openFTP" title="Installation via FTP" class="shard-scroll">
      <template #body>
        <div class="flex flex-col gap-4 p-4">
          <!-- FTP Connection Status -->
          <div class="border border-gray-200 dark:border-gray-800 rounded-lg p-4 bg-gray-50 dark:bg-gray-900">
            <div class="flex items-center justify-between mb-3">
              <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">FTP Connection</h3>
            ...
            <div v-if="ftpIp" class="mb-3">
              <p class="text-xs text-gray-600 dark:text-gray-400 mb-1">Current IP:</p>
              <div class="flex items-center gap-2">
                <code class="text-sm bg-white dark:bg-gray-800 px-2 py-1 rounded">{{ ftpIp }}</code>
                <UButton
            ...
            <div v-if="ftpIps.length > 0" class="mb-3">
              <p class="text-xs text-gray-600 dark:text-gray-400 mb-2">Detected devices:</p>
            ...
            <div>
              ...
              <div v-else class="flex gap-2">
                ...
                <UButton
                  ...
                  variant="ghost"
                  size="sm"
                  @click="showManualIpInput = false; manualIpInput = ''"
                />
              </div>
            </div>
          ...
          <div v-if="currentTransfer" class="border border-gray-200 dark:border-gray-800 rounded-lg p-4">
            ...
            <div class="space-y-2">
              <div class="flex justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">{{ currentTransfer.fileName }}</span>
                <span class="font-medium">{{ currentTransfer.progressPercent.toFixed(1) }}%</span>
              </div>
              
              <UProgress :value="currentTransfer.progressPercent" :max="100" />
              
              <div class="flex justify-between text-xs text-gray-600 dark:text-gray-400">
                <span>{{ formattedFileSize(currentTransfer.bytesTransferred) }} / {{ formattedFileSize(currentTransfer.totalBytes) }}</span>
                <span>{{ formattedTransferSpeed }} • ETA: {{ formattedETA }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="border border-gray-200 dark:border-gray-800 rounded-lg p-4">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg font-semibold">Transfer Queue ({{ transferQueue.length }})</h3>
              <UButton
            ...
            <div v-if="transferQueue.length === 0" class="text-center py-8 text-gray-500">
              No files in queue
            </div>
          ...
          <div class="border border-gray-200 dark:border-gray-800 rounded-lg p-4">
            ...
            <div v-else-if="isScanning" class="text-center py-8">
              ...
              <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">Scanning for games...</p>
            </div>
  
            <div v-else-if="availableGames.length === 0" class="text-center py-8 text-gray-500">
