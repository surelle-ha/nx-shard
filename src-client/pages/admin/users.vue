  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      users.value = usersData.users.map((user: any) => {
        const config = (configsData as UserConfig[])?.find((c) => c.userId === user.id);
        return {
          id: user.id,
          email: user.email,
          name: config?.name || "N/A",
          imageUrl: config?.imageUrl || "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRADwvSebpVf1hSgjHDZyMEpVGxqiAlvE09dA&s",
        ...
        h("div", { class: "flex gap-2" }, [
          h(
            UButton,
            {
              ...
              variant: "soft",
              size: "xs",
              onClick: () => toggleApproval(row.original.id, row.original.isApproved),
            },
